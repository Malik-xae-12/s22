...
export default function Dashboard({ currentUser }: DashboardProps) {
  ...
  const stageStatusData = [
    { name: "Not Started", value: stages.filter((s) => s.status === "not_started").length },
    { name: "In Progress", value: stages.filter((s) => s.status === "in_progress").length },
    { name: "Completed", value: stages.filter((s) => s.status === "completed").length },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Stage Status Chart */}
        <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
          <h3 className="text-lg font-bold text-slate-900 mb-4">Stage Status</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
        <h3 className="text-lg font-bold text-slate-900 mb-4">Task Status Overview</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {taskStatusData.map((item) => (
            <div key={item.name} className="text-center p-4 rounded-lg bg-slate-50">
